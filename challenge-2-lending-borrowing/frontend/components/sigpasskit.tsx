...
export default function SigpassKit({ disableCreate = false }: SigpassKitProps) {
  ...
  function truncateAddress(addr: Address, length = 4) {
  ...
  /* --------------------------- UI SHORT‑CIRCUIT --------------------------- */
  // If creation is disabled, skip all wallet‑creation UI and only render ConnectButton
  if (disableCreate && !address && !account.isConnected) {
    return (
      <ConnectButton
  ...
  /* --------------------------- ORIGINAL UI (modified connect button style) --------------------------- */
  const connectBtn = (
    <ConnectButton className="rounded-xl font-bold text-md hover:scale-105 transition-transform" />
  );
    ...
    return (
      <div className="flex flex-row gap-2 items-center">
        {!wallet && !account.isConnected && !address ? (
                ...
                <div className="flex flex-row gap-8">
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-row gap-4 items-center">
                      <Image src="/rainbowkit-1.svg" alt="icon-1" width={50} height={50} />
                    ...
                    <div className="flex flex-row gap-4 items-center">
                      <Image src="/rainbowkit-2.svg" alt="icon-2" width={50} height={50} />
                      <div className="flex flex-col gap-2">
          ...
          <Button className="rounded-xl font-bold text-md hover:scale-105 transition-transform" onClick={getWallet}>
            Get Wallet
          </Button>
        ) : wallet && !account.isConnected && address ? (
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="border-2 border-primary rounded-xl font-bold text-md hover:scale-105 transition-transform" variant="outline">
        ...
        {!address ? connectBtn : null}
      </div>
    );
  }

  /* --------------------------- MOBILE (unchanged except create flag) --------------------------- */
  return (
    <div className="flex flex-row gap-2 items-center">
      {(!wallet && !account.isConnected && !address) ? (
        disableCreate ? (
